...
и соблюдать переданный `CancellationToken`. `Selftest Check` снова использует
прямой, подтверждённый стендом цикл браузерных попыток; промежуточная обёртка
с блокировкой следующей попытки удалена после регрессии загрузки DUT.
