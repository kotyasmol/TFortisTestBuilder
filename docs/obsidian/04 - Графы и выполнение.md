...
Executor вызывается из UI-команды и сам не переносит все шаги в фон.
Синхронные операции ОС внутри асинхронной ноды поэтому требуют отдельного
worker. У `Selftest Check` за это отвечает `HeadlessBrowserAttemptRunner`:
запуск браузера, чтение, завершение дерева процессов и удаление временного
профиля выполняются вне UI. Ожидание результата ограничено таймаутом и токеном
остановки независимо от того, вернулся ли блокирующий системный вызов.
