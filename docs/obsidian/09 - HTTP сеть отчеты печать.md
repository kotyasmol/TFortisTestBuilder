...
`/test.shtml`. Сравнивает `Dut.firmvare_vers` с целью `0.2.8` (520);
значение `20c` читается как hex 524 и означает 0.2.12; `208` тоже читается
как hex 520 и означает 0.2.8. В профиле сейчас `forceUpdate=false`:
версия 0.2.8 или новее пропускает загрузку. При ручном включении этого
...
повторный selftest и сравнение новой версии с минимумом 0.2.8. При
