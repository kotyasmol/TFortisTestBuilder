...
`HeadlessBrowserAttemptRunner` переносит весь жизненный цикл браузера в
`Task.Run`: в том числе `Process.Start`, `Process.Kill(entireProcessTree)`
и удаление временного профиля, которые могут синхронно задерживаться в ОС.
Нода ждёт результат через отдельный ограниченный `WaitAsync`; токен таймаута
начинается до запуска worker. Поэтому блокирующий запуск или cleanup не
замораживает UI и не мешает исполнителю обработать «Остановить тест».
Если предыдущая попытка после таймаута всё ещё выполняется, новый браузер этой
нодой пока не запускается. Её поздний результат не принимается как selftest,
а позднее исключение наблюдается внутри runner. В логах есть этапы поиска
браузера, запуска, подключения, загрузки, чтения, закрытия и удаления профиля.
Завершение процесса после `Kill` ожидается в фоне не более пяти секунд;
нода не ждёт фоновые операции сверх бюджета попытки. Отдельные системные
операции могут завершиться позже — их принудительное прерывание не гарантируется.
...
уходит на `192.168.0.1:43962`. Порт соответствует `PSW_PORT` в `QTstand_old`,
но пользователь подтвердил, что с портом `6123` MAC после перезапуска всё ещё
оставался старым. Причина отказа на физическом DUT пока не установлена.

В следующем стендовом прогоне 2026-09-09 selftest успешно прочитал 82 поля,
после отправки UDP с `.2:6123` на `.1:43962` возникло сообщение Windows
«Удалённый хост принудительно разорвал существующее подключение». Оно
соответствует `WSAECONNRESET`: при чтении UDP Windows так сообщает об ICMP
`Port Unreachable` на предыдущую отправку. Это указывает на недоступный
UDP-порт (неработающий сервис или сетевой фильтр); доступность веб-страницы
сама по себе не подтверждает доступность обработчика MAC. Код ошибки и
сетевой захват в исходном скриншоте отсутствуют, поэтому причина закрытия
порта/источник ICMP ещё требуют проверки.
[Документация Microsoft recvfrom](https://learn.microsoft.com/en-us/windows/win32/api/winsock/nf-winsock-recvfrom).

Для стенда с несколькими картами в одной подсети добавлен `LocalIp`:
рабочий профиль использует `192.168.0.2` (карта порта 0 по схеме). Если поле
пустое, исходящий адрес определяется по маршруту ОС. Код не перенастраивает
адаптеры. В лог и `SetMac.*` записываются фактические локальные IP/порт,
имя интерфейса и hex отправленного пакета. Занятый порт или отсутствующий
на ПК локальный IP дают явную ошибку; параллельный Qt sender нужно закрыть.

После отправки тот же сокет ждёт подтверждение `mr` (смещения 10–11,
длина от 12 байт, IP источника равен IP DUT). Это признак из
`QTstand_old/Network/network.cpp`; исходники не задают обязательное эхо MAC,
заголовок или порт ответа. Посторонние ответы логируются и игнорируются.
При таймауте команда повторяется тем же сокетом; рабочий профиль допускает
три попытки по 3000 мс с паузой 200 мс. После `mr` повторы прекращаются.

`SetMac.PacketSent` означает только передачу пакета сокету.
`SetMac.Acknowledged`/`SetMac.Success` выставляются лишь после `mr`.
При ошибке отправки `FailOnSendError = true` направляет граф в `False`;
при `false` граф продолжается, но успех не выставляется. Если пакет отправлен,
а ACK не получен, граф продолжается с предупреждением и `Success = false`:
старый автоматический Qt-тест также не требовал ACK, и потеря ответа не должна
блокировать перезапуск. ACK не доказывает сохранение во flash: рабочий граф
по-прежнему перезапускает DUT и сравнивает свежий MAC из selftest с ожидаемым.

`ConnectionReset`/`ConnectionRefused` обрабатываются отдельно только внутри
приёма ответа: логируется ICMP `Port Unreachable` с endpoint и кодом ошибки,
текущая попытка завершается и выполняется следующий повтор. После исчерпания
попыток `PacketSent = true` позволяет графу дойти до reboot/read-back без
ложного `Acknowledged`/`Success`. Ошибки bind, отправки и другие ошибки приёма
не маскируются. `SetMac.ReceiveError`, `SetMac.ReceiveSocketError` и
`SetMac.ReceiveNativeErrorCode` сохраняют последнюю такую ошибку; успешный
повтор очищает итоговую `SetMac.Error`, но оставляет историю ошибки приёма.
