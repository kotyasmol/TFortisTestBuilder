...
3. Исходное `Dut.ups_rez` также берётся из снимка; перед отключением AC
   требуется `Dut.ups_rez == 0`.
...
Подготовка не вызывает `/api/getUpsVoltage`: исходные `ups_det`, `akb_voltage`
и `ups_rez` уже содержатся в только что полученном selftest. Наблюдавшийся на
стенде HTTP 404 именно у `/api/getUpsVoltage` не доказывает отсутствие отдельного
`/api/getUpsStatus`; этот status endpoint использовала старая программа.
