...
1. Свежий `SelftestSnapshot` должен дать `Dut.akb_det == 0`: аккумулятор ещё не
   подключён, AC1 включён.
2. На SIMBAT выполняется `slave 17 / 1706 ← 1` с read-back-проверкой.
3. Свежий снимок должен дать `Dut.akb_det == 1` при работе DUT от сети.
4. На PS-3 выполняется `slave 23 / 1200 ← 0` с проверкой: AC1 выключен.
5. Свежий снимок снова должен дать `Dut.akb_det == 1`. Сам факт успешного
   чтения страницы при выключенном AC1 подтверждает работу DUT от АКБ.
6. AC1 возвращается записью `slave 23 / 1200 ← 1` с проверкой.
7. Свежий снимок должен сохранить `Dut.akb_det == 1` при работе от сети.
8. Имитатор отключается записью `slave 17 / 1706 ← 0` с проверкой.
9. Финальный снимок должен дать `Dut.akb_det == 0`.

Все пять ожиданий используют `SelftestSnapshot`, request timeout `30000` мс,
общий timeout `160000` мс, интервал `5000` мс и `FailOnTimeout = true`.
Фиксированных задержек нет: после каждой записи граф сразу начинает опрос и
завершается при первом свежем ожидаемом состоянии. Старое `Dut.akb_det`
удаляется перед попыткой, поэтому снимок действительно выполняется.
...
регистр SIMBAT `1706`, а состояние подключения читается из `akb_det` тестовой
страницы. Исторические API-ноды остаются только для совместимости профилей.
