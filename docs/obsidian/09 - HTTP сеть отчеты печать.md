...
- через DevTools явно открывает один исходный URL без подмены маршрута;
...
После неуспешной попытки процесс браузера завершается, его временный профиль
удаляется, затем после `PollIntervalMs` запускается новый браузер. Реализация
не хранит отдельный фоновой worker, способный запретить следующую попытку.
