...
4. После отключения AC `Wait Variable Until` в режиме `SelftestSnapshot`
   заново снимает тестовую страницу до `Dut.ups_rez == 1`.
5. После возврата AC такие же свежие снимки ожидают `Dut.ups_rez == 0`.

Оба ожидания рабочего профиля имеют `FailOnTimeout = true`: ошибка снимка,
отсутствие поля или недостижение значения действительно проваливают подтест.
Подготовка не вызывает `/api/getUpsVoltage` и `/api/getUpsStatus`: на части
прошивок эти endpoint отсутствуют, а все три исходных значения уже
содержатся в только что полученном selftest.
