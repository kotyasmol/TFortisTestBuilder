...
`PrintLabelStep` повторяет проверенный алгоритм `RTL_v2`: читает полный номер
из переменной `SerialNumber` и печатает только его — Code128-штрихкодом и
обычным текстом. Значение по умолчанию — четыре одинаковые этикетки на
`TSC TE310`.
...
Хотя в старых названиях классов встречается `ZebraLabel`, фактическая команда
`RTL_v2` имеет формат EPL (`N`, `q354`, `B...`, `A...`, `P1,1`), а не ZPL.
Текущая программа воспроизводит этот формат для этикетки 30x10 мм при 300 dpi,
кодирует команду в Windows-1251 и не добавляет MAC, модель или служебные поля.
Четыре полных EPL-команды передаются одним RAW-заданием.
