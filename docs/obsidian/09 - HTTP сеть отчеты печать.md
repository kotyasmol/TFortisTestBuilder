...
`SN: SSSSS` и Code128 payload `032SSSSS`. Полный номер преобразуется через
offset `3200000`; также допускается непосредственный ввод короткого номера
`0..65535`. В поставляемом ручном мини-профиле режим включён и ZPL-команда
повторяется четыре раза в одном RAW-задании. Основной производственный профиль
пока сохраняет прежний EPL-режим. После первой физической печати штрихкод
сдвинут на 30 точек влево (`^FO480,117`), чтобы восстановить правое тихое поле;
координаты текстовых строк не изменялись.
