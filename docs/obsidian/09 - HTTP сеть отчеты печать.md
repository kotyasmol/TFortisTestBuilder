...
3. Пока AC ещё включён, `Read HTTP Variable` делает preflight
   `/api/getUpsStatus`, записывает свежий Integer в ту же `Dut.ups_rez` и
   требует `Dut.ups_rez == 0`. Если API недоступен, тест завершается до снятия
   питания и не смешивает ошибку endpoint с ошибкой перехода на АКБ.
4. После отключения AC `Wait Variable Until` в режиме `HttpGet` опрашивает
   `/api/getUpsStatus` до свежего `Dut.ups_rez == 1`.
5. После возврата AC такой же скалярный опрос ожидает `Dut.ups_rez == 0`.

Оба ожидания рабочего профиля имеют request timeout `5000` мс, интервал
`5000` мс, общий timeout `160000` мс и `FailOnTimeout = true`. Пока DUT
переключается или загружается, сетевые ошибки не завершают ноду немедленно:
она удаляет старое `Dut.ups_rez` и повторяет GET до ответа `0/1` либо общего
таймаута. Поэтому недоступность страницы сразу после снятия AC допустима, но
устаревшее значение из исходного selftest не может дать ложный успех.

Этот динамический участок повторяет транспорт старой Qt-программы, где
`GetUpsStatus()` обращался к `/api/getUpsStatus`, а цикл делал паузу 5 секунд.
Полный `SelftestSnapshot` для переходов не используется: browser-попытка ждёт
DOM и фиксированные 10 секунд, поэтому значительно реже проверяет состояние.

Подготовка не вызывает `/api/getUpsVoltage`: `ups_det` и `akb_voltage` уже
содержатся в только что полученном selftest. Наблюдавшийся на стенде HTTP 404
именно у `/api/getUpsVoltage` не доказывает отсутствие отдельного
`/api/getUpsStatus`; status endpoint проверяется preflight-запросом ещё при AC
и затем используется в обоих переходах, как в старой программе.
