...
как hex 520 и означает 0.2.8. В профиле
`forceUpdate=true`, поэтому загрузка выполняется даже при более новой версии.
Если версия уже равна целевой, загрузка пропускается и при `forceUpdate=true`.
...
повторный selftest и сравнение новой версии точно с 0.2.8. POST использует
...
Это протокол старого PSW; к Pro он не применяется. Аппаратный прогон пока
не проведён.
