...
4. После отключения AC и двухсекундной стабилизации выполняются прямые live-read
   проверки SIMBAT24: slave 17, discharge voltage `1707 > 0` и discharge current
   `1708 > 0`. Это подтверждает реальное питание DUT от батарейной цепи до
   длительного сетевого ожидания.
5. Только после физических проверок `Wait Variable Until` в режиме
   `SelftestSnapshot` получает свежую тестовую страницу до `Dut.ups_rez == 1`.
6. После возврата AC такой же свежий снимок ожидает `Dut.ups_rez == 0`.

Оба ожидания рабочего профиля имеют request timeout и общий timeout `300000`
мс, интервал между browser-попытками `5000` мс и `FailOnTimeout = true`.
Перед каждым снимком старое `Dut.ups_rez` удаляется, поэтому исходный `0/1` не
может дать ложный успех.

Стенд подтвердил, что `/api/getUpsVoltage`, `/api/getUpsStatus` и IRP API на
текущей прошивке отвечают HTTP 404. Поэтому рабочий профиль не использует эти
endpoint. Старая Qt-программа действительно опрашивала `/api/getUpsStatus`, но
этот транспорт применим только к прошивкам, где такой endpoint существует.
Универсальные `HttpGet` и legacy UPS-ноды остаются в runtime для других профилей.
