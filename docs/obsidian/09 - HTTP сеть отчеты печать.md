...
удаляется, затем после `PollIntervalMs` запускается новый браузер. Весь lifecycle
попытки, включая снимок DOM и очистку, идёт вне UI-потока. Внешний цикл ждёт его
завершения, поэтому две browser-попытки не пересекаются.
