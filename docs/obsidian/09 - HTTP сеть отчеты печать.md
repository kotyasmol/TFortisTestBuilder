...
`/test.shtml`. Сравнивает `Dut.firmvare_vers` с целью `0.2.13`; значение
`20c` читается как hex 524, цель соответствует `20d`/525. Если версия уже
не ниже целевой, загрузка пропускается. Иначе до первого HTTP-запроса
проверяются путь, имя, расширение `.img` и SHA-256. Затем выполняются
...
повторный selftest и сравнение новой версии. При провале шага тест останавливается.
