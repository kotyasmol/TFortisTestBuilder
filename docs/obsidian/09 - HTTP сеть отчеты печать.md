...
2. `Dut.akb_voltage` из того же снимка проверяется диапазоном.
3. Исходное `Dut.ups_rez` проверяется из того же снимка; перед отключением AC
   требуется `0`.
4. После отключения AC и двухсекундной стабилизации выполняются прямые live-read
   проверки SIMBAT24: slave 17, discharge voltage `1707` в диапазоне
   `12000..27000` мВ и discharge current `1708 > 0` мА. Это подтверждает
   реальную отдачу энергии DUT от батарейной цепи.
5. `Wait Variable Until` в режиме `HttpReachable` ждёт HTTP 2xx от корня DUT и
   сохраняет `Dut.http_ready_on_battery = true`. В сочетании с током/напряжением
   это функционально подтверждает, что DUT работает при снятом AC.
6. Для возврата включается AC1, затем отключается SIMBAT. После двух секунд
   `HttpReachable` должен снова получить HTTP 2xx и сохранить
   `Dut.http_ready_on_ac = true`; тем самым DUT подтверждается уже без батареи.

Оба ожидания имеют request timeout `5000` мс, общий timeout `160000` мс,
интервал `5000` мс и `FailOnTimeout = true`. Body страницы не разбирается:
важен факт доступности web-интерфейса. Старое Boolean-значение удаляется перед
каждой попыткой и не может дать ложный успех.

Стенд подтвердил, что `/api/getUpsVoltage`, `/api/getUpsStatus` и IRP API на
текущей прошивке отвечают HTTP 404, а после перехода на АКБ тестовая страница
300 секунд отвечает HTML без `<selftest>`/`<settings>`. Поэтому динамический
`ups_rez` недоступен обоими старыми способами. Рабочий профиль использует
наблюдаемые функциональные признаки: питание/ток SIMBAT и доступность DUT при
снятом AC, затем доступность DUT после отключения SIMBAT. Старая Qt-программа
действительно опрашивала `/api/getUpsStatus`, но этот транспорт применим только
к прошивкам, где endpoint существует.
