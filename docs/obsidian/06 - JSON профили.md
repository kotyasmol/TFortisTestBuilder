...
в ноль и следующий свежий снимок обязан показать соответствующий `sensor_1=0`
или `sensor_2=0`. Следующий датчик запускается только после обеих проверок
предыдущего. Каждый снимок проверяет готовность (`init_ok=1`, числовой `dev_type`),
...
Проверка датчика вскрытия с установленной шторкой ожидает `Dut.sensor_0=0`
по указанию оператора (инверсия прежнего ожидания `1`); это же изменение
внесено в диагностический черновик. Парсер сохраняет исходное значение DUT,
проверки `sensor_1/2` не инвертированы.
