...
Для фонового исполнения браузера `Selftest Check` дополнительные JSON-поля
не нужны: `timeoutMs` остаётся общим сроком ожидания DUT. Внутренний предел
одной браузерной попытки — 30000 мс (или меньше при близком общем deadline),
включая запуск и закрытие. Незавершённая старая попытка блокирует запуск
следующего браузера этой нодой, но не блокирует UI или остановку теста.
