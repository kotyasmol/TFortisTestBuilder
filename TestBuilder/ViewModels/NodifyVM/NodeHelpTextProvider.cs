    ...
    internal static class NodeHelpTextProvider
    {
        private static readonly IReadOnlyDictionary<Type, string> HelpByType = new Dictionary<Type, string>
        {
            ...
            [typeof(GetSerialNumberFromServerNodeViewModel)] = """
                ...
                Production вызывает getSerialNum с Device и CPU ID, как старый Qt; getExistsSerialNum здесь не используется.
            ...
            [typeof(SetProMacNodeViewModel)] = """
                ...
                MAC var - переменная с полным MAC-адресом из Build MAC From Serial.
                Board version - имя модели, передаваемое вторым аргументом bat-файлу.
                Timeout ms - максимальное время работы bat; зависший процесс будет остановлен.
                Fail on error - перейти по False, если bat не запустился, завис, вернул ненулевой код или не напечатал SUCCESSFUL.
                Третий аргумент формируется автоматически: текущий Unix timestamp в секундах.
                После перезагрузки сохранение MAC обязательно проверяется отдельным selftest.
                """,
            ...
            [typeof(CheckIo2SensorsAndRelayNodeViewModel)] = """
                ...
                До любых проверок нода записывает 0 в OUT1 (1500) и OUT2 (1501), затем подтверждает оба нуля чтением.
                Пока исходные нули не подтверждены, включение выходов запрещено.
                Для Sensor1 и Sensor2 нода замыкает IO-2 OUT1/OUT2 и ждет sensor_1/sensor_2 = 1 в свежем selftest.
                Для реле сначала выключается тестовый режим DUT и сбрасывается IN1 (1507) записью 0, как в старом стенде.
                Затем set_mb_output=1 → IN1 = 1; выключение → сброс IN1 → подтверждение 0.
                В любом исходе OUT1/OUT2 выключаются. Команды реле и сброс IN1 выполняются только при Check relay.
                """,
            ...
            [typeof(PrintLabelNodeViewModel)] = """
                Печатает этикетки RAW-командами через Windows spooler.
            ...
            [typeof(OperatorActionNodeViewModel)] = """
                ...
                Конструкции {SerialNumber} или {Dut.default_mac} заменяются значениями переменных теста.
                Отсутствующая переменная явно показывается как <не задано: ИмяПеременной>.
                Продолжить идет по одноимённому выходу, Отмена - по выходу "Отмена".
                """,
        };
