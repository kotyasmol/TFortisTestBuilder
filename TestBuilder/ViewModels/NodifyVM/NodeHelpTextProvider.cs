    ...
    internal static class NodeHelpTextProvider
    {
        private static readonly IReadOnlyDictionary<Type, string> HelpByType = new Dictionary<Type, string>
        {
            ...
            [typeof(UpdatePswFirmwareNodeViewModel)] = """
                ...
                Наличие файла .img проверяется до очистки памяти коммутатора.
                Force update выполняет загрузку даже если текущая версия новее целевой.
                Последовательность: GET /clear.shtml, пауза 10 с, POST образа,
                пауза 10 с, GET подтверждения, пауза 40 с, новый selftest.
                В логе [FW] видны HTTP-коды, длительности, паузы и этап отказа.
