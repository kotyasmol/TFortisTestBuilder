    ...
    internal static class NodeHelpTextProvider
    {
        private static readonly IReadOnlyDictionary<Type, string> HelpByType = new Dictionary<Type, string>
        {
            ...
            [typeof(SendUdpSetMacPacketNodeViewModel)] = """
                ...
                Local IP - IPv4 сетевой карты стенда; пусто = маршрут ОС. На многопортовом стенде укажи IP нужной карты.
                Local port - исходящий порт (6123 как в QTstand); 0 = автоматический.
                MAC var - переменная, из которой берется MAC-адрес.
                Timeout ms - лимит отправки и ожидания ответа mr на каждую попытку.
                Attempts - максимум попыток; после ответа mr повторы прекращаются.
                Repeat delay - пауза между повторами.
                Fail on send error - считать ошибку отправки провалом теста.
                Если пакет отправлен, отсутствие mr даёт предупреждение, но не блокирует перезапуск и проверку MAC.
                Ответ mr подтверждает команду; сохранение MAC проверяется отдельным selftest после перезапуска.
                """,
