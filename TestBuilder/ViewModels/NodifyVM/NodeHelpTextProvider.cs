    ...
    internal static class NodeHelpTextProvider
    {
        private static readonly IReadOnlyDictionary<Type, string> HelpByType = new Dictionary<Type, string>
        {
            ...
            [typeof(GetSerialNumberFromServerNodeViewModel)] = """
                Безопасно получает серийный номер устройства с сервера.
                ...
                Сначала вызывается getExistsSerialNum: найденный номер используется повторно.
                Новый номер запрашивается только после точного ответа 0 и только один раз за запуск ноды.
                Ошибка проверки или выдачи останавливает ноду без повторной выдачи, чтобы не создать дубль.
                Output - имя переменной, куда сохранить серийный номер.
                Timeout - лимит одного запроса.
                Retry - количество повторов проверки существующего номера и задержка между ними.
