...
QByteArray QtTelnetAuthNull::authStep(const QByteArray &data)
{
    ...
    unsigned char buf[8] = {Common::IAC, Common::SB, Common::Authentication,
...
void QtTelnetPrivate::parseSubTT(const QByteArray &data)
{
    ...
    const unsigned char c1[4] = { Common::IAC, Common::SB,
                                  Common::TerminalType, Common::IS};
    sendCommand((char *)c1, sizeof(c1));
    sendString("UNKNOWN");
    const unsigned char c2[2] = { Common::IAC, Common::SE };
    sendCommand((char *)c2, sizeof(c2));
}
...
void QtTelnetPrivate::sendWindowSize()
{
    ...
    const unsigned char c[9] = { Common::IAC, Common::SB, Common::NAWS,
                                 (w & 0x00ff), (w >> 8), (h & 0x00ff), (h >> 8),
                                 Common::IAC, Common::SE };
    sendCommand((char *)c, sizeof(c));
}
...
{
    const unsigned char c[3] = { Common::IAC, operation, option };
    sendCommand((char *)c, 3);
}
...
void QtTelnet::sendControl(Control ctrl)
{
    ...
    const unsigned char command[2] = {Common::IAC, c};
    d->sendCommand((char *)command, sizeof(command));
    if (sendsync)
        sendSync();
}
